...
class foroModel {
    ...
    function registrar_topico($titulo, $bloqueado, $fecha, $idf, $idadm) {
        $sql = "INSERT INTO topico VALUES(0, '$titulo', '$bloqueado', '$fecha', '$idf', '$idadm', 0)";
        $msg=$this->get_cntmsg($idf); $cant=$msg[0][0]+1;
        $sql2 = "UPDATE foro SET cant_mensajes='$cant' WHERE idf='$idf'";
        if($this->con->conexion->query($sql)){ 
    ...
    function get_cntmsg($idf) {
        $sql = "SELECT cant_mensajes AS cant FROM foro WHERE idf='$idf'";
